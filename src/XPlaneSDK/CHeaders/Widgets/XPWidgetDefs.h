...
typedef enum {
   ...
   * classes* provided with the library. *
   *                                                                            *
   * Properties 1000 - 1099 are for widget class 0, 1100 - 1199 for widget
   * class* 1, etc. */
...
typedef enum {
  ...
  /* The message is sent just to the target, but goes to every callback, even
   * if* it is handled. */
...
typedef enum {
   ...
   * the mouse       * location. You will continue to receive these until the
   * mouse button is     * released. You may receive multiple mouse state
   * messages with the same mouse* position. You will receive mouse drag events
   * even if the mouse is dragged  * out of your current or original bounds at
   * the time of the mouse down.      *
   *                                                                            *
   * Dispatching: Direct *
   *                                                                            *
   * Param 1: A pointer to an XPMouseState_t containing the mouse status. */
   ...
   * * provided with the library with 1000 - 1099 for widget class 0, 1100 -
   * 1199 * for widget class 1, etc. Message IDs 10,000 and beyond are for
   * plugin use. */
